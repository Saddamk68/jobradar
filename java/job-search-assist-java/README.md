...
Job-Search_Assist crawls ATS job boards, stores job postings, and analyzes job descriptions via an external Python service. The scheduled ingestion runs periodically and saves job postings and analysis results.
...
- Run (from project root):
    - `mvn spring-boot:run`
    - Or run the generated jar: `java -jar target/job-search_assist-1.0-SNAPSHOT.jar`
...
- `jobsearchassist.python.client.url` (or similar property used by `PythonClient`)
